...
class TicagaTickets extends TicagaSupportModel
{
     ...
     * @return mixed An stdClass object representing the ticket, or false if none exist
     */
    public function doesTicketBelongToClient($ticket_user_id)
    {
    ...
    private function getUserInfoById($id)
    {
        ...
        $ipaddress = $this->get_client_ip_server();
        $resp = $this->TicagaSettings->callAPI("customers/get/" . $id, $apiURL,$apiEmail,$apiKey);
     ...
     */
    public function getDepartmentsByID($department_id): mixed
    {
		$company_id = Configure::get('Blesta.company_id');
        $apiKey = $this->getAPIInfoByCompanyId($company_id)->api_key;
		$apiURL = $this->getAPIInfoByCompanyId($company_id)->api_url;
		...
		$resp = $this->TicagaSettings->callAPI("departments/get_by_id/" . $department_id,$apiURL,$apiEmail,$apiKey);
        $resp_test = $this->TicagaSettings->validateAPISuccessResponse($resp);
    ...
    /**
     * Retrieves department info by department Slug
     *
     * @param int $departmentid The Slug of the department whose department info to fetch
     * @return Json/boolean response 
     */
    public function getDepartmentsBySlug($department_slug): mixed
    {
		$company_id = Configure::get('Blesta.company_id');
        $apiKey = $this->getAPIInfoByCompanyId($company_id)->api_key;
		$apiURL = $this->getAPIInfoByCompanyId($company_id)->api_url;
    ...
    public function getDepartmentsForPublicUseOnly()
    {
		...
		$resp = $this->TicagaSettings->callAPI("departments/public",$apiURL,$apiEmail,$apiKey);
		$resp_test = $this->TicagaSettings->validateAPISuccessResponse($resp);
    ...
    public function getDepartmentsForClientsUseOnly()
    {
		...
		$resp = $this->TicagaSettings->callAPI("departments/customers",$apiURL,$apiEmail,$apiKey);
		$resp_test = $this->TicagaSettings->validateAPISuccessResponse($resp);
     ...
     */
    public function getPrioritiesHighAllowed(?string $department_slug = null)
    {
            // Grabs the department information
			$department_information = $this->getDepartmentsBySlug($department_slug);

            if($department_information == '')
            {
                $department_information = $this->getDepartmentsById($department_slug);
            }

            // Splits the array to remove the untidy array result
            $department_array = $department_information['department'][0];
            
			if ($department_array['allows_high_priority'] == '1')
			{
			    return true;
			} else {
			    return false;
